...
// ── Phase 2 types ────────────────────────────────────────────────
type RoadmapStep = { phase: string; actions: string[] };
type ScholarshipItem = { name: string; amount: string; eligibility: string; apply: string };
type GovtSchemeItem = { scheme: string; benefit: string; link: string };
type AlternateCareer = { title: string; why: string; entryExam: string };

type Phase2Result = {
  phase?: string;
  careerId?: string;
  hero?: { icon: string; title: string; tagline: string; stream: string; matchScore: number; oneLiner: string };
  counselorTake?: { heading: string; content: string };
  salaryMap?: { entry: string; mid: string; senior: string; top: string; cityNote: string; timeToMidSalary: string; salaryInsight: string };
  demandData?: { growthPercent: number; growthLabel: string; growthSource: string; indiaShortfall: string; automationRisk: string; topHiringCities: string[]; topHiringCompanies: string[] };
  educationPath?: { primaryRoute: { degree: string; duration: string; exams: string[]; topColleges: string[]; note: string }; alternateRoutes: { route: string; why: string; cost: string; timeline: string }[]; pgOptions: string[] };
  skillsRequired?: { technical: string[]; soft: string[]; certifications: string[] };
  careerRoadmap?: { heading: string; steps: RoadmapStep[] };
  scholarships?: { heading: string; list: ScholarshipItem[] };
  govtSchemes?: GovtSchemeItem[];
  honestChallenges?: { heading: string; points: string[] };
  alternateIfNotPossible?: { heading: string; careers: AlternateCareer[] };
  personalizedClosing?: { heading: string; content: string };
};
  ...
  return (
    <div style={{ background: color.bg, border: `1px solid ${color.border}`, borderRadius: 16, padding: 24, marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
        {career.previewExams?.slice(0, 3).map((e, i) => (
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '16px 16px 60px' }}>
      ...
      {d && (
        <div className="result-card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>{d.topHiringCities?.map((c, i) => chip(c, i))}</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{d.topHiringCompanies?.map((c, i) => chip(c, i))}</div>
        </div>
      )}
      ...
      {ed && (
        <div className="result-card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>{ed.primaryRoute.exams?.map((e, i) => chip(e, i, 'rgba(124,58,237,0.12)', 'rgba(124,58,237,0.3)'))}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 8 }}>{ed.primaryRoute.topColleges?.map((c, i) => <div key={i} style={{ fontSize: '0.8rem', opacity: 0.8 }}>• {c}</div>)}</div>
          ...
          {ed.pgOptions && (<><div style={{ fontSize: '0.75rem', opacity: 0.5, marginTop: 10, marginBottom: 6 }}>POSTGRAD OPTIONS</div><div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>{ed.pgOptions.map((p, i) => <div key={i} style={{ fontSize: '0.8rem', opacity: 0.75 }}>• {p}</div>)}</div></>)}
        </div>
      )}
      ...
      {sk && (
        <div className="result-card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>{sk.technical?.map((t, i) => chip(t, i))}</div>
          <div style={{ fontSize: '0.72rem', opacity: 0.5, marginBottom: 6 }}>SOFT SKILLS</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>{sk.soft?.map((t, i) => chip(t, i))}</div>
          <div style={{ fontSize: '0.72rem', opacity: 0.5, marginBottom: 6 }}>CERTIFICATIONS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>{sk.certifications?.map((c, i) => <div key={i} style={{ fontSize: '0.8rem', opacity: 0.75 }}>• {c}</div>)}</div>
        </div>
      )}
          ...
          {(phase1Result.streamOptions || []).map((opt, i) => (
            <div key={i} style={{ background: i === 0 ? 'rgba(99,102,241,0.1)' : 'rgba(16,185,129,0.08)', border: `1px solid ${i === 0 ? 'rgba(99,102,241,0.3)' : 'rgba(16,185,129,0.25)'}`, borderRadius: 16, padding: 24 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                {opt.topCareers?.map((c, ci) => <span key={ci} style={{ padding: '3px 10px', borderRadius: 20, fontSize: '0.73rem', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' }}>{c}</span>)}
              </div>
        ...
        {sc && (
          <div style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: 16, padding: 24, marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {sc.keyExams?.map((e, i) => <span key={i} style={{ padding: '4px 12px', borderRadius: 20, fontSize: '0.75rem', background: 'rgba(124,58,237,0.12)', border: '1px solid rgba(124,58,237,0.3)' }}>{e}</span>)}
            </div>
          </div>
        )}
