...
export function getStreakWhatsAppLink(days: number): string {
  const text = `I've been studying for ${days} days straight on Learnova AI! 🔥 Join me: ${getAppUrl()}`
...
export function getExamWhatsAppLink(score: number, total: number, subject: string): string {
  const text = `I just scored ${score}/${total} on a ${subject} mock test on Learnova AI! 📚 Try it free: ${getAppUrl()}`
  return getWhatsAppLink(text)
}

// Generate WhatsApp share link for business validation
export function getBusinessWhatsAppLink(score: string): string {
  const text = `My startup idea scored ${score}/10 on Learnova AI's India validator! 🚀 ${getAppUrl()}`
  return getWhatsAppLink(text)
}
...
function getAppUrl(): string {
  ...
  return process.env.NEXT_PUBLIC_APP_URL || 'https://learnova.ai'
}
