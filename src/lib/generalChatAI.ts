...
async function fetchWithRetry(url: string, options: RequestInit, provider: string, retryCount = 0): Promise<Response> {
  const response = await fetch(url, options);

  if (response.status === 429 && retryCount < MAX_RETRIES) {
    console.warn(`[AI] ${provider} rate limited (429), retrying... (${retryCount + 1}/${MAX_RETRIES})`);
    await wait(1000 * (retryCount + 1));
    return fetchWithRetry(url, options, provider, retryCount + 1);
  }
...
async function callGroq(messages: AIMessage[], maxTokens: number): Promise<string> {
  ...
  try {
    ...
    if (err.message.includes('429')) {
      console.warn(`[AI] Groq ${primaryModel} failed with 429, trying fallback ${fallbackModel}`);
...
async function callGemini(messages: AIMessage[], maxTokens: number): Promise<string> {
  ...
  try {
    ...
    if (err.message.includes('429')) {
      console.warn(`[AI] Gemini ${primaryModel} failed with 429, trying fallback ${fallbackModel}`);
      return await makeCall(fallbackModel);
    }
    throw err;
  }
...
async function callOpenAI(messages: AIMessage[], maxTokens: number): Promise<string> {
  ...
  const fallbackModel = 'gpt-3.5-turbo'; // Extra fallback if mini fails
  ...
  try {
    ...
    if (err.message.includes('429')) {
      console.warn(`[AI] OpenAI ${primaryModel} failed with 429, trying fallback ${fallbackModel}`);
      return await makeCall(fallbackModel);
    }
    throw err;
  }
